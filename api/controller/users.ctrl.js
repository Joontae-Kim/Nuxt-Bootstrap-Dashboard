...
const { createUsersTraffic, searchUser, createRandomAcitivy, createRandomPurchaseList, commonStatics } = require('../lib/users')
...
const statics = async (req, res, next) => {
  try {
    ...
    const total = commonStaticsSet.total
    ...
    res.status(200).send({ ...commonStaticsSet, activeUsers, groupByUsers, weekSerial: serialTraffic, timeSerial: timeSeriesTraffic })
  } catch (err) {
    console.log(`        ~ err => `, err)
