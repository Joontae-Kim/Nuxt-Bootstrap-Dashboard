...
export function customTooltipsGenerator (tooltipModel) {
  ...
  const spacing = tooltipModel.width * 0.55
  const distanceToisLeft = tooltipModel.x + tooltipModel.width < chartPosition.width - tooltipModel.width
  const distanceToRight = tooltipModel.x > tooltipModel.width + spacing
  const isCenter = distanceToisLeft && distanceToRight

  // Display, position, and set styles for font
  tooltipEl.style.top = chartPosition.y + window.pageYOffset + tooltipModel.caretY + 10 + 'px'
  const leftDefaultPosition = chartPosition.x + window.pageXOffset + tooltipModel.caretX
  tooltipEl.style.left = isCenter
    ? `${leftDefaultPosition - spacing}px`
    : distanceToisLeft
