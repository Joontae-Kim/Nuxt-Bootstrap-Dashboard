...
export function customTooltipsGenerator (tooltipModel) {
  console.log(`customTooltipsGenerator ~ `)
  // console.log(` ~ `)
  ...
  const [spacingHorizon, spacingVertical] = [tooltipModel.width * 0.55, tooltipModel.height]

  // Compute X-axis distance
  const distanceToisLeft = tooltipModel.x + tooltipModel.width < chartPosition.width - tooltipModel.width
  const distanceToRight = tooltipModel.x > tooltipModel.width + spacingHorizon
  ...
  const distanceYCommonCompute = tootipVerticalMiddle - spacingVertical
  const distanceToTop = Math.abs(distanceYCommonCompute - chartPosition.height - 10)
  ...
  tooltipEl.style.left = isCenter
    ? `${leftDefaultPosition - spacingHorizon}px`
    : distanceToisLeft
      ? `${leftDefaultPosition}px`
      : `${(chartPosition.x + window.pageXOffset + tooltipModel.caretX) - tooltipModel.width}px`
